    ...
    module.label = function(value) {
        ...
        function setLinkCoordinate() {
            edges.forEach(edge => {
                ...
                const sourceNode = vertices.find(node => node.id === source);
                const targetNode = vertices.find(node => node.id === target);
        
                // 设置连接的坐标
                edge.points = [
                    { x: sourceNode.x + sourceNode.width, y: sourceNode.y + sourceNode.height }, // 开始点在父节点底部的右侧
                    { x: targetNode.x, y: targetNode.y } // 结束点在子节点的左侧
                ];
            });
        }
