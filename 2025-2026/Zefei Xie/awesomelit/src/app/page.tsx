...
export default function HomePage() {
  ...
  const createAutoPipelineMutation = useMutation({
    mutationFn: (query: string) => pipelineApi.runFull(query),
    onSuccess: (data) => {
      router.push(`/auto/${data.pipeline_id}`);
    },
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (query.trim()) {
      if (mode === 'auto') {
        createAutoPipelineMutation.mutate(query);
      } else {
  ...
  const isLoading = createPipelineMutation.isPending || createAutoPipelineMutation.isPending;
  const error = createPipelineMutation.error || createAutoPipelineMutation.error;
                ...
                <div className="flex gap-4 p-4 bg-gray-50 rounded-xl">
                  ...
                  >
                    <div className="flex flex-col items-center gap-2">
                      <span className="text-2xl">⚡</span>
                      <span>Auto Mode</span>
                      <span className="text-xs opacity-80">End-to-end automation</span>
                    </div>
                  </button>
                </div>
                ...
                >
                  {isLoading ? (
                      <>
                        ...
                        <span>{mode === 'auto' ? 'Running automation...' : 'Starting pipeline...'}</span>
                      </>
                  ) : (
                      <>
                        <span>{mode === 'auto' ? 'Run Automatically' : 'Start Research'}</span>
